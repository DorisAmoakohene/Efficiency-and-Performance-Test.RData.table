---
title: 'gmean na.rm=TRUE is much slower than na.rm=FALSE #4849'
output: html_document
date: "2024-02-15"
---

gmean na.rm=TRUE is much slower than na.rm=FALSE #4849
library(data.table) ## 1.13.5
setDTthreads(0L) ## 40
set.seed(108)
N = 1e9L
K = 1e2L
DT = list()
DT[["id3"]] = factor(sample(sprintf("id%010d",1:(N/K)), N, TRUE))
DT[["v3"]] =  round(runif(N,max=100),6)
setDT(DT)


```{r}

library(atime)
library(ggplot2)
library(data.table)
library(directlabels)

```

```{r}
  tdir <- tempfile()
  dir.create(tdir)
  git2r::clone("https://github.com/Rdatatable/data.table", tdir)
  
```




system.time(nat <- DT[, .(v3=mean(v3, na.rm=TRUE)), by=id3, verbose=TRUE])
```{r,warning=FALSE,message=FALSE}
atime.list.4849 <- atime::atime_versions(
  pkg.path=tdir,
  pkg.edit.fun=function(old.Package, new.Package, sha, new.pkg.path){
    pkg_find_replace <- function(glob, FIND, REPLACE){
      atime::glob_find_replace(file.path(new.pkg.path, glob), FIND, REPLACE)
    }
    Package_regex <- gsub(".", "_?", old.Package, fixed=TRUE)
    Package_ <- gsub(".", "_", old.Package, fixed=TRUE)
    new.Package_ <- paste0(Package_, "_", sha)
    pkg_find_replace(
      "DESCRIPTION", 
      paste0("Package:\\s+", old.Package),
      paste("Package:", new.Package))
    pkg_find_replace(
      file.path("src","Makevars.*in"),
      Package_regex,
      new.Package_)
    pkg_find_replace(
      file.path("R", "onLoad.R"),
      Package_regex,
      new.Package_)
    pkg_find_replace(
      file.path("R", "onLoad.R"),
      sprintf('packageVersion\\("%s"\\)', old.Package),
      sprintf('packageVersion\\("%s"\\)', new.Package))
    pkg_find_replace(
      file.path("src", "init.c"),
      paste0("R_init_", Package_regex),
      paste0("R_init_", gsub("[.]", "_", new.Package_)))
    pkg_find_replace(
      "NAMESPACE",
      sprintf('useDynLib\\("?%s"?', Package_regex),
      paste0('useDynLib(', new.Package_))
  },
  seconds.limit=1,
  N=10^seq(3,9),
  setup={
    DT = list()
    DT[["id3"]] = factor(sample(sprintf("id%010d",1:(N/100)), N, TRUE))
    DT[["v3"]] =  round(runif(N,max=100),6)
    setDT(DT)
  },
  expr= data.table:::`[.data.table`(DT, , .(v3=mean(v3, na.rm=TRUE)), by=id3, verbose=TRUE),
  "Before"="616dfc5093b0f7eea6aafbc1ecc744a541088b12",#https://github.com/Rdatatable/data.table/tree/616dfc5093b0f7eea6aafbc1ecc744a541088b12
  "Regression"="5b04b0ef9691f71fcb9f13658c0f7b5b30944a1f",#https://github.com/Rdatatable/data.table/commit/5b04b0ef9691f71fcb9f13658c0f7b5b30944a1f
  "fixed"="ec1259af1bf13fc0c96a1d3f9e84d55d8106a9a4")#https://github.com/Rdatatable/data.table/commit/ec1259af1bf13fc0c96a1d3f9e84d55d8106a9a4
```


```{r}
png()
plot(atime.list.4849)+
  labs(title = "gmean na.rm=TRUE is much slower than na.rm=FALSE #4849")

png("atime.list.4849.png")
plot(atime.list.4849)+
  labs(title = "gmean na.rm=TRUE is much slower than na.rm=FALSE #4849")
dev.off()
```

